...
로그인은 `/ask` 의 **제출 순간**에만 걸립니다 — 허브 · 설명 면(`/how` ·
`/documents` · `/principals`) · 직원 면은 로그인 없이 열립니다. 로그인이
권한이 아니라 **요금 게이트**이기 때문입니다: `/ask` 뒤에만 LLM 이 있고,
나머지 화면은 전부 DB 읽기라 요금이 0원입니다. 지키려는 것과 막는 것을
어긋나게 두지 않으려고 게이트를 그 한 자리로 좁혔습니다. 알리스트
(`ASK_ALLOWLIST`) 밖 계정은 하루 `ASK_DAILY_LIMIT`(기본 10)회로 질의가
제한되고, 넘기면 429 를 받습니다.

관리자 면(`/logs` · `/admin`)은 이메일 알리스트가 아니라 **고른 페르소나의
역할**이 정합니다 — `남감사` 가 `auditor` 입니다. 역할은 `principals.role`
에 있고 서버가 `GET /principals` 로 내려줍니다. 프론트에 이름→역할 맵을
두지 않는다는 것을 `backend/tests/test_persona_role.py` 가 고정합니다.

열람 이력은 행을 전부 보여주되 **다른 방문자의 질의 원문만** 백엔드가
가립니다(`core/access/masking.py`). 거르지 않고 가리는 이유는, 거르면
방문자에게 그 화면이 백지가 되면서 *"시연용으로 채운 데이터가 아닙니다"*
라는 그 화면의 주장까지 함께 사라지기 때문입니다.
...
**Faithfulness — 무엇을 재고 무엇을 안 재는가.** LLM 심판을 쓰지 않습니다.
답변이 인용한 조항 코드가 실제 검색 결과(`hits`)에 있는지로 잽니다
(`frontend/lib/cited.ts`). 화면은 인용을 셋으로 가릅니다:

| 갈래 | 뜻 |
|---|---|
| 인용한 조항 | 답변이 가리켰고 검색 결과에도 있다 |
| 연관 자료 | 검색은 됐지만 답변이 인용하지 않았다 |
| 지어낸 인용 | 답변이 가리켰는데 **검색 결과에 없다** — 모델이 만든 번호다 |

셋째가 있으면 화면이 경고로 드러냅니다. 예전에는 검색 상위 k 를 통째로
"근거 조항" 이라고 불러서, 답변이 "찾지 못했습니다" 인데 그 아래 근거가
일곱 개 붙는 일이 있었습니다 — 근거가 아닌 것을 근거라고 부른 것입니다.

**재지 않는 것**: 인용한 조항의 *내용* 이 답변과 맞는지. 번호가 실재하는지만
봅니다. `eval/` 의 지표(recall · MRR · nDCG)도 여전히 검색 품질만 잽니다.
