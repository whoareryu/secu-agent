...
샙니다 — 그러나 프로덕션 경로에는 닿지 않습니다. `backend/tests/test_demo_isolation.py`
의 `test_api_에서_demo_를_아는_파일은_정확히_하나다` 가 `demo` 를 import 하는
`api/` 아래 파일이 `api/demo.py` 하나뿐임을 부분집합이 아니라 집합 동일성으로
강제합니다.
