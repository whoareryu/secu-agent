...
def test_순서가_호출마다_안정적이다(코퍼스, 임베딩):
    """`naive_search.py` 의 바깥 ORDER BY 가 실제로 순서를 고정하는지 본다.

    바깥 ORDER BY 없이 JOIN 만 하면 Postgres 가 순서를 보장하지 않는다 —
    실측: 조인 전략을 merge join 으로 고정하면 같은 입력에도 다른 순서가
    나온다. 화면(LeakCompare.tsx 의 조항_묶기)은 이 순서를 그대로 칩으로
    그리므로, 같은 질의를 두 번 부르면 같은 id 순서가 나와야 한다.
    """
