...
def test_본문이_빈_조항은_청크를_만들지_않는다():
    """빈 청크는 임베딩·저장까지 가서, 검색에 걸리면 제목만 출력한다.

    실코퍼스에는 없지만(실측: 빈 청크 0건) 조항 코드 줄 바로 뒤에 다음
    조항 코드가 오는 문서면 생긴다. 만들지 않는 편이 조용히 흘리는 것보다
    낫다.
    """
    assert chunk_clauses([Clause(code="1.1.1", title="제목", text="")]) == []
    assert chunk_clauses([Clause(code="1.1.1", title="제목", text="   \n\n  ")]) == []


def test_60자가_넘는_조항_제목이_유실되지_않는다():
    """상한을 넘으면 그 줄이 조항으로 안 잡힌다 — **조항 전체가 앞 조항의
    본문으로 삼켜지고 조항 코드가 사라진다.** 에러 없이 데이터가 없어진다.

    실측: 지금 코퍼스의 최장 제목은 21자이고, 상한을 200 으로 올려도
    조항 102개·청크 314개가 그대로다. 코퍼스를 흔들지 않으면서 여유만
    넓힌다.
    """
    긴_제목 = "가" * 70
    조항들 = split_clauses(f"1.1.1 짧은 제목\n첫 조항 본문\n1.1.2 {긴_제목}\n둘째 조항 본문\n")

    assert [c.code for c in 조항들] == ["1.1.1", "1.1.2"]
    assert 조항들[1].title == 긴_제목
