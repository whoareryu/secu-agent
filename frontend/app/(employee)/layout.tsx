...
export default async function EmployeeLayout({ children }: { children: React.ReactNode }) {
  ...
  // 한다(스펙 §2.1).
  //
  // hasPersona 에 true 를 박지 않는다. 바로 위 guard 가 이미 null 을 걸러
  // 오늘은 참이지만, 그렇게 쓰면 불변식이 성립하는 자리에서 떨어져 나와
  // 위 판정이 바뀌는 날 조용히 거짓이 된다.
  const 항목 =
    guard({ surface: "admin", hasPersona: name !== null, role }) === "ok"
