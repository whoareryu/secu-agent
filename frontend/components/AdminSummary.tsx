...
export default function AdminSummary() {
  ...
  // 질의 수: 고유 (query, persona, 시각) 조합 수. 마스킹이 들어온 뒤로 남의
  // 행에서는 query 가 전부 같은 가림 문구라(backend/core/access/masking.py),
  // 그 행들을 실제로 가르는 것은 ts 다 — 한 번의 질의가 남긴 여러 행은 같은
  // 트랜잭션에서 들어가 같은 ts 를 갖고, 다른 질의는 다른 ts 를 갖는다.
  // 그러니 키를 줄이겠다고 ts 를 빼면 다른 방문자의 질의가 전부 한 건으로
  // 접힌다. 열람된 서류: 고유 청크 수 —
